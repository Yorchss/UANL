...
$modelo = $_POST['modelo'];
$serie = $_POST['serie'];
$fecha = strftime("%d/%m/%Y", strtotime("now"));
...
$user = $_SESSION["user"];
$user = strval($user);
$creador = $nombre." ".$apell." - ".$user;
$creador = strval($creador);
?>
<!doctype html>
<html lang="es">
...
<body>
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <?php
            $fallo = false;
            $sql = 'INSERT INTO Accesorio (tipo_Accesorio,modelo_Accesorio,serie_Accesorio,fecha_Accesorio,creador_Accesorio,marca_Accesorio) VALUES (?,?,?,?,?,?)';
            $params = array($tipoAccesorio,$modelo,$serie,$fecha,$creador,$marca);
            ...
            if ($fallo == true) {
                echo "<script> alert('No se ha podido insertar los datos con exito. Vuelva a intentarlo por favor.'); window.location.href='Accesorios.php'; </script>";
            } else {
                echo "<script> alert('Se han insertado los datos de manera exitosa.'); window.location.href='Accesorios.php'; </script>";
            }
            ?>
        </div>
    </div>
